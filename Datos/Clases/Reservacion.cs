...
namespace Datos.Clases
{
    public class Reservacion
    {
        ...
        public void Insertar(Comunes.Estructuras.Reservacion pDatos)
        {
            string vSQL = string.Empty;
            Conexion.Conexion vConnexion = new Conexion.Conexion();
            try
            {
                vSQL += "INSERT INTO dbo.RESERVACION (IdReservacion, IdPersona, IdTipoPersona, Saldo, IdTipoPago, IdEstadoReservacion, FechaReservacion, Total) ";
                vSQL += "VALUES(" + pDatos.IdReservacion.ToString() + ", " + pDatos.IdPersona.ToString() + ", " + pDatos.IdTipoPersona.ToString() + ", " + pDatos.Saldo.ToString() + ", " + pDatos.IdTipoPago.ToString() + ", " + pDatos.IdEstadoReservacion.ToString() + ", " + pDatos.FechaReservacion.ToString() + ", " + pDatos.Total.ToString() + "" + ")";
        ...
        public void Eliminar(List<Filtro> pFiltros)
        {
            ...
            try
            {
                if (pFiltros != null && pFiltros.Count != 0)
                {
                    vSQL += "DELETE FROM dbo.RESERVACION";
                    vSQL += vFiltros.CrearWhere(pFiltros);
                }
                vConnexion.Ejecutar(vSQL, "BASEDATOS");
            }
        ...
        public void Actualizar(List<Filtro> pFiltros, List<Campos> pCampos)
        {
            ...
            try
            {
                ...
                vSQL += "UPDATE dbo.RESERVACION" + vQuery;
                vSQL += vFiltros.CrearWhere(pFiltros);
                vConnexion.Ejecutar(vSQL, "BASEDATOS");
            }
            catch (Exception ex)

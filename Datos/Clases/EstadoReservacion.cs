    ...
    public class EstadoReservacion
    {
        ...
        public DataTable Selecccionar(List<Filtro> pFiltros)
        {
            ...
            try
            {
                ...
                vSQL += ",DescripcionEstadoReservacion ";
                vSQL += "FROM dbo.ESTADO_RESERVACION";
        ...
        public void Insertar(Comunes.Estructuras.EstadoReservacion pDatos)
        {
            string vSQL = string.Empty;
            Conexion.Conexion vConnexion = new Conexion.Conexion();
            try
            {
                vSQL += "INSERT INTO dbo.ESTADO_RESERVACION (IdEstadoReservacion, DescripcionEstadoReservacion) ";
                vSQL += "VALUES(" + pDatos.IdEstadoReservacion.ToString() + ", " + "'" + pDatos.DescripcionEstadoReservacion + "'" + ")";
        ...
        public void Eliminar(List<Filtro> pFiltros)
        {
            ...
            try
            {
                if (pFiltros != null && pFiltros.Count != 0)
                {
                    vSQL += "DELETE FROM dbo.ESTADO_RESERVACION";
                    vSQL += vFiltros.CrearWhere(pFiltros);
                }
                vConnexion.Ejecutar(vSQL, "BASEDATOS");
            }
        ...
        public void Actualizar(Comunes.Estructuras.EstadoReservacion pDatos, List<Filtro> pFiltro)
        {
            ...
            try
            {
                vSQL += "UPDATE dbo.ESTADO_RESERVACION SET " + "IdEstadoReservacion = " + pDatos.IdEstadoReservacion.ToString() + ", " + "DescripcionEstadoReservacion = " + "'" + pDatos.DescripcionEstadoReservacion + "'";
                if (pFiltro != null)
                {
        ...
        public void Actualizar(List<Filtro> pFiltros, List<Campos> pCampos)
        {
            ...
            try
            {
                ...
                vSQL += "UPDATE dbo.ESTADO_RESERVACION" + vQuery;
                vSQL += vFiltros.CrearWhere(pFiltros);
                vConnexion.Ejecutar(vSQL, "BASEDATOS");
            }
            catch (Exception ex)

    ...
    public class EstadoAsiento
    {
        ...
        public DataTable Selecccionar(List<Filtro> pFiltros)
        {
            ...
            try
            {
                ...
                vSQL += ",DescripcionEstadoAsiento ";
                vSQL += "FROM dbo.ESTADO_ASIENTO";
        ...
        public void Insertar(Comunes.Estructuras.EstadoAsiento pDatos)
        {
            string vSQL = string.Empty;
            Conexion.Conexion vConnexion = new Conexion.Conexion();
            try
            {
                vSQL += "INSERT INTO dbo.ESTADO_ASIENTO (IdEstadoAsiento, DescripcionEstadoAsiento) ";
                vSQL += "VALUES(" + pDatos.IdEstadoAsiento.ToString() + ", " + "'" + pDatos.DescripcionEstadoAsiento + "'" + ")";
        ...
        public void Eliminar(List<Filtro> pFiltros)
        {
            ...
            try
            {
                if (pFiltros != null && pFiltros.Count != 0)
                {
                    vSQL += "DELETE FROM dbo.ESTADO_ASIENTO";
                    vSQL += vFiltros.CrearWhere(pFiltros);
                }
                vConnexion.Ejecutar(vSQL, "BASEDATOS");
            }
        ...
        public void Actualizar(Comunes.Estructuras.EstadoAsiento pDatos, List<Filtro> pFiltro)
        {
            string vSQL = string.Empty;
            Filtro vFiltros = new Filtro();
            Conexion.Conexion vConnexion = new Conexion.Conexion();
            try
            {
                vSQL += "UPDATE dbo.ESTADO_ASIENTO SET " + "IdEstadoAsiento = " + pDatos.IdEstadoAsiento.ToString() + ", " + "DescripcionEstadoAsiento = " + "'" + pDatos.DescripcionEstadoAsiento + "'";
                if (pFiltro != null)
                {
        ...
        public void Actualizar(List<Filtro> pFiltros, List<Campos> pCampos)
        {
            ...
            try
            {
                ...
                vSQL += "UPDATE dbo.ESTADO_ASIENTO" + vQuery;
                vSQL += vFiltros.CrearWhere(pFiltros);
                vConnexion.Ejecutar(vSQL, "BASEDATOS");
            }
            catch (Exception ex)

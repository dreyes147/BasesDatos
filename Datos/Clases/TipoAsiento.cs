...
namespace Datos.Clases
{
    public class TipoAsiento
    {
        ...
        public void Insertar(Comunes.Estructuras.TipoAsiento pDatos)
        {
            string vSQL = string.Empty;
            Conexion.Conexion vConnexion = new Conexion.Conexion();
            try
            {
                vSQL += "INSERT INTO dbo.TIPO_ASIENTO (IdTipoAsiento, DescripcionTipoAsiento) ";
                vSQL += "VALUES(" + pDatos.IdTipoAsiento.ToString() + ", " + "'" + pDatos.DescripcionTipoAsiento + "'" + ")";
        ...
        public void Eliminar(List<Filtro> pFiltros)
        {
            ...
            try
            {
                if (pFiltros != null && pFiltros.Count != 0)
                {
                    vSQL += "DELETE FROM dbo.TIPO_ASIENTO";
                    vSQL += vFiltros.CrearWhere(pFiltros);
                }
                vConnexion.Ejecutar(vSQL, "BASEDATOS");
            }
        ...
        public void Actualizar(Comunes.Estructuras.TipoAsiento pDatos, List<Filtro> pFiltro)
        {
            string vSQL = string.Empty;
            Filtro vFiltros = new Filtro();
            Conexion.Conexion vConnexion = new Conexion.Conexion();
            try
            {
                vSQL += "UPDATE dbo.TIPO_ASIENTO SET " + "IdTipoAsiento = " + pDatos.IdTipoAsiento.ToString() + ", " + "DescripcionTipoAsiento = " + "'" + pDatos.DescripcionTipoAsiento + "'";
                if (pFiltro != null)
                {
        ...
        public void Actualizar(List<Filtro> pFiltros, List<Campos> pCampos)
        {
            ...
            try
            {
                ...
                vSQL += "UPDATE dbo.TIPO_ASIENTO" + vQuery;
                vSQL += vFiltros.CrearWhere(pFiltros);
                vConnexion.Ejecutar(vSQL, "BASEDATOS");
            }
            catch (Exception ex)

...
{
    public partial class mEstadoPersona : Form
    {
        public mEstadoPersona()
        {
            InitializeComponent();
        }
        ...
        private void CargarVista()
        {
            ...
            Negocio.Clases.Estado vProx = new Negocio.Clases.Estado();
            List<Comunes.Filtros.Filtro> vFiltros = new List<Comunes.Filtros.Filtro>();
            ListViewItem vItem = new ListViewItem();
            try
            {
                vResultados = vProx.Selecccionar(vFiltros);
                ltvInformacion.Items.Clear();
                foreach (DataRow vRow in vResultados.Rows)
                {
                    vItem = ltvInformacion.Items.Add(vRow["IdEstado"].ToString());
                    vItem.SubItems.Add(vRow["DescripcionEstado"].ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private void mEstadoPersona_Load(object sender, EventArgs e)
        {
            try
            {
                Inicializar();
        ...
        {
            Comunes.Estructuras.Estado vEstructura = new Comunes.Estructuras.Estado();
            Negocio.Clases.Estado vNegocio = new Negocio.Clases.Estado();
            List<Comunes.Filtros.Filtro> vFiltros = new List<Comunes.Filtros.Filtro>();
            try
            {
                if (txtDescripcion.Text.Trim() != string.Empty)
                {
                    ...
                    vEstructura.DescripcionEstado = txtDescripcion.Text;
                    switch (vModo)
                    {
                        case "A":
                            vEstructura.IdEstado = 0;
                            vNegocio.Insertar(vEstructura);
                            break;
                        case "M":
                            vEstructura.IdEstado = Convert.ToInt32(lblId.Text);
                            vFiltros.Add(new Comunes.Filtros.Filtro("IdEstado", "=", Convert.ToInt32(lblId.Text)));

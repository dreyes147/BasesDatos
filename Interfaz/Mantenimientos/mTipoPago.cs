...
{
    public partial class mTipoPago : Form
    {
        public mTipoPago()
        {
            InitializeComponent();
        ...
        private void CargarVista()
        {
            ...
            Negocio.Clases.TipoPago vPrx = new Negocio.Clases.TipoPago();
            ...
            try
            {
                ...
                {
                    vItem = ltvInformacion.Items.Add(vRow["IdTipoPago"].ToString());
                    vItem.SubItems.Add(vRow["DescripcionTipoPago"].ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private void mTipoPago_Load(object sender, EventArgs e)
        {
            try
            {
                Inicializar();
        ...
        {
            Comunes.Estructuras.TipoPago vEstructura = new Comunes.Estructuras.TipoPago();
            Negocio.Clases.TipoPago vNegocio = new Negocio.Clases.TipoPago();
            List<Comunes.Filtros.Filtro> vFiltros = new List<Comunes.Filtros.Filtro>();
            try
            {
                if (txtDescripcion.Text.Trim() != string.Empty)
                {
                    ...
                    vEstructura.DescripcionTipoPago = txtDescripcion.Text;
                    switch (vModo)
                    {
                        case "A":
                            vEstructura.IdTipoPago = 0;
                            vNegocio.Insertar(vEstructura);
                            break;
                        case "M":
                            vEstructura.IdTipoPago = Convert.ToInt32(lblId.Text);
                            vFiltros.Add(new Comunes.Filtros.Filtro("IdTipoPago", "=", Convert.ToInt32(lblId.Text)));

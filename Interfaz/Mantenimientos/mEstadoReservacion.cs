...
{
    public partial class mEstadoReservacion : Form
    {
        public mEstadoReservacion()
        {
            InitializeComponent();
        }
        ...
        private void CargarVista()
        {
            ...
            Negocio.Clases.EstadoReservacion vPrx = new  Negocio.Clases.EstadoReservacion();
            ...
            try
            {
                ...
                {
                    vItem = ltvInformacion.Items.Add(vRow["IdEstadoReservacion"].ToString());
                    vItem.SubItems.Add(vRow["DescripcionEstadoReservacion"].ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private void mEstadoReservacion_Load(object sender, EventArgs e)
        {
            try
            {
                Inicializar();
        ...
        {
            Comunes.Estructuras.EstadoReservacion vEstructura = new Comunes.Estructuras.EstadoReservacion();
            Negocio.Clases.EstadoReservacion vNegocio = new Negocio.Clases.EstadoReservacion();
            List<Comunes.Filtros.Filtro> vFiltros = new List<Comunes.Filtros.Filtro>();
            try
            {
                if (txtDescripcion.Text.Trim() != string.Empty)
                {
                    ...
                    vEstructura.DescripcionEstadoReservacion = txtDescripcion.Text;
                    switch (vModo)
                    {
                        case "A":
                            vEstructura.IdEstadoReservacion = 0;
                            vNegocio.Insertar(vEstructura);
                            break;
                        case "M":
                            vEstructura.IdEstadoReservacion = Convert.ToInt32(lblId.Text);
                            vFiltros.Add(new Comunes.Filtros.Filtro("IdEstadoReservacion", "=", Convert.ToInt32(lblId.Text)));

...
{
    public partial class mGenero : Form
    {
        public mGenero()
        {
            InitializeComponent();
        ...
        private void CargarVista()
        {
            ...
            Negocio.Clases.Genero vPrx = new Negocio.Clases.Genero();
            ...
            try
            {
                ...
                {
                    vItem = ltvInformacion.Items.Add(vRow["IdGenero"].ToString());
                    vItem.SubItems.Add(vRow["DescripcionGenero"].ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private void mGenero_Load(object sender, EventArgs e)
        {
            try
            {
                Inicializar();
        ...
        {
            Comunes.Estructuras.Genero vEstructura = new Comunes.Estructuras.Genero();
            Negocio.Clases.Genero vNegocio = new Negocio.Clases.Genero();
            List<Comunes.Filtros.Filtro> vFiltros = new List<Comunes.Filtros.Filtro>();
            try
            {
                if (txtDescripcion.Text.Trim() != string.Empty)
                {
                    ...
                    vEstructura.DescripcionGenero = txtDescripcion.Text;
                    switch (vModo)
                    {
                        case "A":
                            vEstructura.IdGenero = 0;
                            vNegocio.Insertar(vEstructura);
                            break;
                        case "M":
                            vEstructura.IdGenero = Convert.ToInt32(lblId.Text);
                            vFiltros.Add(new Comunes.Filtros.Filtro("IdGenero", "=", Convert.ToInt32(lblId.Text)));

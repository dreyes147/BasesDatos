...
{
    public partial class mEstadoAsiento : Form
    {
        public mEstadoAsiento()
        {
            InitializeComponent();
        }
        ...
        private void CargarVista()
        {
            ...
            Negocio.Clases.EstadoAsiento vEstado = new Negocio.Clases.EstadoAsiento();
            List<Comunes.Filtros.Filtro> vFiltros = new List<Comunes.Filtros.Filtro>();
            ListViewItem vItem = new ListViewItem();
            try
            {
                vResultados = vEstado.Selecccionar(vFiltros);
                ltvInformacion.Items.Clear();
                foreach (DataRow vRow in vResultados.Rows)
                {
                    vItem = ltvInformacion.Items.Add(vRow["IdEstadoAsiento"].ToString());
                    vItem.SubItems.Add(vRow["DescripcionEstadoAsiento"].ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private void mEstadoAsiento_Load(object sender, EventArgs e)
        {
            Inicializar();
        }
        ...
        {
            Comunes.Estructuras.EstadoAsiento vEstructura = new Comunes.Estructuras.EstadoAsiento();
            Negocio.Clases.EstadoAsiento vNegocio = new Negocio.Clases.EstadoAsiento();
            List<Comunes.Filtros.Filtro> vFiltros = new List<Comunes.Filtros.Filtro>();
            try
            {
                if (txtDescripcion.Text.Trim() != string.Empty)
                {
                    ...
                    vEstructura.DescripcionEstadoAsiento = txtDescripcion.Text;
                    switch (vModo)
                    {
                        case "A":
                            vEstructura.IdEstadoAsiento = 0;
                            vNegocio.Insertar(vEstructura);
                            break;
                        case "M":
                            vEstructura.IdEstadoAsiento = Convert.ToInt32(lblId.Text);
                            vFiltros.Add(new Comunes.Filtros.Filtro("IdEstadoAsiento", "=", Convert.ToInt32(lblId.Text)));
        ...
        private void mEstadoAsiento_Load_1(object sender, EventArgs e)
        {
            Inicializar();
        }
    }

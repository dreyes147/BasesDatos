...
{
    public partial class mTipoPersona : Form
    {
        public mTipoPersona()
        {
            InitializeComponent();
        ...
        private void CargarVista()
        {
            ...
            Negocio.Clases.TipoPersona vPrx = new Negocio.Clases.TipoPersona();
            ...
            try
            {
                ...
                {
                    vItem = ltvInformacion.Items.Add(vRow["IdTipoPersona"].ToString());
                    vItem.SubItems.Add(vRow["DescripcionTipoPersona"].ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private void mTipoPersona_Load(object sender, EventArgs e)
        {
            try
            {
                Inicializar();
        ...
        {
            Comunes.Estructuras.TipoPersona vEstructura = new Comunes.Estructuras.TipoPersona();
            Negocio.Clases.TipoPersona vNegocio = new Negocio.Clases.TipoPersona();
            List<Comunes.Filtros.Filtro> vFiltros = new List<Comunes.Filtros.Filtro>();
            try
            {
                if (txtDescripcion.Text.Trim() != string.Empty)
                {
                    ...
                    vEstructura.DescripcionTipoPersona = txtDescripcion.Text;
                    switch (vModo)
                    {
                        case "A":
                            vEstructura.IdTipoPersona = 0;
                            vNegocio.Insertar(vEstructura);
                            break;
                        case "M":
                            vEstructura.IdTipoPersona = Convert.ToInt32(lblId.Text);
                            vFiltros.Add(new Comunes.Filtros.Filtro("IdTipoPersona", "=", Convert.ToInt32(lblId.Text)));

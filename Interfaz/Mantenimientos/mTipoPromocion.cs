...
{
    public partial class mTipoPromocion : Form
    {
        public mTipoPromocion()
        {
            InitializeComponent();
        }
        ...
        private void CargarVista()
        {
            ...
            Negocio.Clases.TipoPromocion vPrx = new Negocio.Clases.TipoPromocion();
            ...
            try
            {
                ...
                {
                    vItem = ltvInformacion.Items.Add(vRow["IdTipoPromocion"].ToString());
                    vItem.SubItems.Add(vRow["DescripcionTipoPromocion"].ToString());
                    vItem.SubItems.Add(vRow["ValorPromocion"].ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private void mTipoPromocion_Load(object sender, EventArgs e)
        {
            try
            {
                Inicializar();
        ...
        {
            Comunes.Estructuras.TipoPromocion vEstructura = new Comunes.Estructuras.TipoPromocion();
            Negocio.Clases.TipoPromocion vNegocio = new Negocio.Clases.TipoPromocion();
            List<Comunes.Filtros.Filtro> vFiltros = new List<Comunes.Filtros.Filtro>();
            try
            {
                if (txtDescripcion.Text.Trim() != string.Empty && txtValor.Text.Trim() != string.Empty)
                {

                    vEstructura.DescripcionTipoPromocion = txtDescripcion.Text;
                    vEstructura.ValorPromocion = txtValor.Text;
                    switch (vModo)
                    {
                        case "A":
                            vEstructura.IdTipoPromocion = 0;
                            vNegocio.Insertar(vEstructura);
                            break;
                        case "M":
                            vEstructura.IdTipoPromocion = Convert.ToInt32(lblId.Text);
                            vFiltros.Add(new Comunes.Filtros.Filtro("IdTipoPromocion", "=", Convert.ToInt32(lblId.Text)));
